    ...
    do {
        ...
        try {
            switch (choice) {
                ...
                case 5: addTeamToLeague(loggables); break;
                case 6: removeTeamFromLeague(loggables); break;
                case 0: break;
            }
...
void ScreenManager::removeLeague(const std::vector<Loggable*>& loggables) {
    ...
    if (!league->getTeams().empty()) {
        throw ValidationException("A liga (" + league->getName() + ") nao pode ser removida, pois possui times vinculados. Remova todos os times primeiro.");
    }
